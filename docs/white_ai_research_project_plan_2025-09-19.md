...
1.  **Authentication Token:**
    ...
    - _Expected Answer Format:_ The token itself, or clear instructions on how White.ai (as a deployed Cloudflare Pages application) can securely access it (e.g., via Cloudflare Pages Environment Variables, Cloudflare Workers Secrets).
2.  **Cloudflare Worker Authentication Logic:**
    ...
    - _Expected Answer Format:_ A brief description of the validation mechanism.
3.  **Rate Limiting Policy:**
    ...
    - _Expected Answer Format:_ Specific numbers (e.g., "10 requests per 60 seconds per IP").
4.  **Detailed Error Handling:**
    ...
    - _Expected Answer Format:_ JSON examples for each status code, matching the `{"error": {"code": "string", "message": "string"}}` schema.
5.  **AI Model & Prompt Engineering (for Context/Debugging):**
    ...
    - _Expected Answer Format:_ Model name (e.g., "@cf/meta/llama-3.1-8b-instruct"), and a brief description of the prompting approach.
...
1.  **"The Principle of Symbiotic Disbelief" Content:**
    ...
    - _Expected Answer Format:_ The full text of the markdown file.
...
3.  **Other Canonical Documents:**
    ...
    - _Expected Answer Format:_ Full markdown text for each additional document.
4.  **Metadata for Each Document:**
    ...
    - _Expected Answer Format:_ A list of metadata objects for each document.
